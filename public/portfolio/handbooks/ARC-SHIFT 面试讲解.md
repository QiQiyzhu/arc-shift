...
> **Decision Lab 增量验收：151 单元 / 34 开发浏览器 / 4 生产浏览器通过。** 新增 5 项几何与偏好规则测试、2 项生产交互流程，typecheck / lint / build 均通过；记录见 [本轮结果](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/qa/engineering/decision-lab/delivery-summary.json)。另有 [30 秒 / 3 分钟 / 8 分钟深度讲解](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/interview-deep-dive.md)。下文 M8 数量是保留的历史基线，不是当前新增后的总数。
...
职责边界是核心：World 保存可变运行态；Engine 控制阶段与调用顺序；战斗规则决定命中和结算；Phaser 负责输入采样、固定步调度与表现；React 通过 Engine 命令推进非实时选择，HUD 约每 80ms 刷新。核心系统不导入 Phaser，Node 可以直接运行同一套战斗逻辑。这里是集中式 World 加系统函数，并非严格 ECS，也没有服务端权威模拟。[架构说明](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/architecture.md)、[Engine](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/game/engine.ts)、[Scene](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/game/scene.ts)、[UI 生命周期](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/ui/GameApp.tsx)。
...
**读码顺序**：先读 [types](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/game/types.ts)、World 与 Engine 的 `update/advance`；再顺着一次开火追到 `shoot → updateProjectiles → hitEnemy → dropLoot`；然后看 Grid 与 brute 对照测试；随后读 Replay、Save、Content；最后读 Scene/ActionInput、React/音频的挂载与释放。每一轮都先预测行为，再运行对应夹具核对，不以背诵类名代替理解。P/Q 节列出正式的十加五文件清单。
...
对应 [源码树](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src)、[测试树](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests)、[浏览器用例](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/e2e)、[工作流](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/.github/workflows/ci.yml)。目录里存在工具不等于该工具已部署到公开地址。
...
主存档读取失败回退安全默认，写入异常返回 `false`；解析旧存档时舍弃损坏 checkpoint 并尽量保留合法 meta/settings。新路线存档重新生成规范图，再校验已走路径，而不是直接相信导入的任意房间对象。局终结算有 `settled` 防止重复记账，但没有服务端防篡改承诺。[save.ts](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/core/save.ts)、[World](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/game/world.ts)、[存档兼容测试](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/save-compatibility.test.ts)。

## D. RAG Pipeline

**N/A。** 游戏不调用 embedding、向量库、检索模型、reranker 或基于检索的回答生成。协议、敌人、图鉴和叙事碎片是随客户端发布的目录与规则；UI 中的查看、筛选不构成 RAG。策划配置的 `ID → 对应数值消费者` 是数据依赖，不应包装为知识检索流水线。[协议目录](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/cards/catalog.ts)、[成长与叙事目录](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/progression/catalog.ts)。

## E. Agent Workflow

**LLM Agent workflow：N/A。** 游戏没有线上模型规划器、工具调用循环、模型审批、token 预算或供应商路由。开发过程使用 AI coding，与运行中的游戏敌人 AI 是两回事。

实际的“AI”是 [enemy-ai.ts](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/ai/enemy-ai.ts) 中由计时器、距离、阶段和固定步驱动的敌人/Boss 规则。Engine 的 `menu/reward/transition/bossIntro/playing/paused/map/event/gameover/victory` 控制游戏阶段；Enemy 的行为状态与 Boss HP 阶段控制招式。暂停保存 `resumePhase`，从暂停返回原来的入场或战斗阶段，不能随意跳到 `playing`。角色死亡后后续危险区/拾取不应继续让结算产生矛盾。[Engine](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/game/engine.ts)、[战斗回归](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/regressions.test.ts)。

可以用“有限状态机”“阶段控制”“规则驱动敌人”描述它，不能说实现了自主推理 Agent、行为树编辑器或训练策略网络。复杂敌人移动使用局部避障，并非通用导航网格寻路。

## F. MCP 设计

实际是浏览器可选的 `document.modelContext` 集成，实现在 [webmcp.ts](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/ui/webmcp.ts)。若宿主没有该对象，安装函数返回空清理函数，游戏照常运行。它既不启动独立 MCP Server，也没有 stdio/SSE/Streamable HTTP 传输层；不能套用 OpsPilot 的服务端 MCP 设计。
...
Replay 声明 `replayVersion=1`、`gameVersion=1.0-engineering.1` 和 `contentVersion=builtin-2026-09-10-tuning1`，不兼容版本被拒绝。参数包使用 schemaVersion；主存档与输入配置各有版本。依赖锁文件、阶段检查脚本、CI 的 source SHA/environment、原始报告和失败留档形成可复查链条。报告日期、环境与 sourceDirty 必须一起解读。[Replay 版本](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/replay/replay.ts)、[阶段检查脚本](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/scripts/check-stage.mjs)、[CI 定义](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/.github/workflows/ci.yml)。

AI-assisted 开发记录的是谁生成/审查/修正了什么，而非生产模型运营。[AI 开发记录](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/ai-development-log.md)。
...
[回放说明](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/replay.md)、[内容说明](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/content-editor.md)、[调试器](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/debugger.md)、[稳定性证据](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/stability.md)、[音频生命周期测试](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/audio-lifecycle.test.ts)。这些机制提供可复现的合同检查，不证明所有玩家输入序列、所有硬件或所有小时级资源泄漏都已覆盖。
...
[导入与资源限制](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/replay/replay.ts)、[Content schema](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/content/schema.ts)、[生产构建报告](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/qa/engineering/bundle.json)。依赖审计曾检出 6 个受影响包条目（4 high、2 moderate，涉及 sharp/mocker 工具链）；定向升级后 [实际审计为 0](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/qa/engineering/audit-m7.json)，完整 M7 回归通过。audit 0 只表示该次依赖数据库检查无已报告项，不是无漏洞证明。[修复与暴露范围](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/dependency-security.md)。

## J. Test 数量与实际结果

当前新增实验室后为 **151 单元、34 DEV 浏览器、4 生产浏览器**。首次生产用例因种子选择器使用了错误的精确标签及文件名假设而失败；保留 [失败记录](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/qa/engineering/decision-lab-first-failure/production.json)，修正定位并增加历史返回行为后四项均通过。实验室没有修改游戏规则，也没有将候选检查减少率报告为帧率收益。
...
第一次 Linux CI [34447501573](https://github.com/QiQiyzhu/arc-shift/actions/runs/34447501573) 在源码 `5cf8270` 失败，开发 smoke 为 2 passed / 5 failed；编译、unit、soak、build 和生产步骤成功不代表整个 CI 成功。该 [原始失败](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/qa/engineering/ci-first-failure/browser.json) 保留。后续 Linux 和 M8 整体结果以 [验收记录](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/qa-report.md) 的准确源 SHA、运行链接及 smoke/full 范围为准。

本地 M7 第二轮的 2 个失败也保留：控制器接管用例朝实体墙移动，以及奖励夹具早于场景创建。[失败报告与截图](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/qa/engineering/m7-input-final)。修复夹具前提后完整重跑通过，没有删去断言或关闭地形。
...
真实存在的是暴力碰撞与 Grid 在相同固定输入下的算法对照，以及配置数值提取前后的默认等价测试。它们分别证明被测碰撞语义保持、默认属性推导保持；不能证明自主 Agent 成功率、玩家乐趣或所有构筑平衡。[空间对照测试](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/spatial-grid.test.ts)、[参数等价测试](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/content.test.ts)。

## M. Performance 真实结果

**M2 固定步 Node 夹具**：seed 73129，300 tick 预热、1500 tick 测量，60Hz 规则更新，真实敌人逻辑与混合武器，高 HP 敌人维持负载。下表是记录在当时 Windows/i9-13900HX 环境的局部观测；其他任务可能同时运行，没有置信区间，也不外推到所有设备。[方法与局限](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/performance-v2.md)、[汇总 JSON](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/qa/engineering/spatial-comparison.json)、[before](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/qa/engineering/engine-baseline.json)、[after](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/qa/engineering/engine-spatial-final.json)。
...
首版 Grid 的 250 敌人 P95 **4.135ms，发生回退**：反复移除/插入格子与重新分配排序候选的成本盖过收益。缓存实体占据格子边界、只有跨格才刷新候选后才得到最终结果。[首版原始记录](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/qa/engineering/engine-spatial-first.json)。CPU 采样中，brute 的 distance/分离/segmentHits 自耗时占比约 30.1%/14.3%/13.7%；Grid 的热点转为 query 37.9%、insert 12.9%。采样包含启动/TS 编译与全部负载，不能当单个函数的精确运行时间。
...
28 敌人均帧下降，100 接近，250 平均略升但 P95 帧时间变差；不能说“稳定 60 FPS”。Scene 的 50ms delta 截断让慢帧推进较少模拟 tick，所以浏览器的累计 query 与弹体峰值不能当作等量固定工作负载比较。原始记录：[28 before](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/qa/engineering/browser-baseline-28.json)/[after](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/qa/engineering/browser-spatial-final-28.json)，[100 before](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/qa/engineering/browser-baseline-100.json)/[after](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/qa/engineering/browser-spatial-final-100.json)，[250 before](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/qa/engineering/browser-baseline-250.json)/[after](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/qa/engineering/browser-spatial-final-250.json)。

**M6 长模拟不是三小时真人/浏览器 soak**：6 种子每组 108,000 tick，即每组模拟 30 分钟、合计 648,000 tick/模拟 3 小时；实际墙钟每组约 6.04–15.48 秒。夹具为不死训练玩家与 28 个高 HP 敌人，无渲染、无音频、无实时等待。弹体峰值 161–169、危险区峰值 17，全部 0 pool misses；每 600 tick 与最后 tick 检查有限数、实体和池边界。报告包含 heap/RSS 快照，但不能证明没有泄漏，尤其不能覆盖 GPU、DOM 或音频资源。[soak 脚本](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/scripts/soak.cjs)、[真实报告](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/qa/engineering/soak-long.json)。

**历史 M6 游戏产物体积**：文件清单合计 3,073,830 bytes，其中 JS 1,664,223 bytes。该数字是本地产物字节数；gzip 样本也不是实际网络传输体积，不包含 CDN、缓存、协议和设备体验推断。后续 bundle 报告会随构建刷新，M8 还包含按需加载的公开手册与录屏，因此不能与纯游戏历史总量直接比较。[当前 bundle.json](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/qa/engineering/bundle.json)。

复现时使用锁文件安装，固定 Node 版本与种子，分别运行 brute/grid，保留全部 JSON。浏览器用隔离环境、多次采样和设备说明；不要挑选单个最好截图。命令以 [performance-v2.md](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/performance-v2.md) 和 [stability.md](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/stability.md) 为准。
...
| 多项共鸣说明遮住重抽按钮 | 奖励区域缺少受限高度内的滚动；修复后加 900/768 高度真实指针回归 | [M5 失败](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/qa/engineering/m5-debugger/browser.json)、[成功](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/qa/engineering/m5-debugger-final/browser.json)、[布局用例](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/e2e/reward-layout.spec.ts) |
| 未知 legacy Boss 可绕过存档检查 | 旧房间对象引用漏校验；丢弃坏 checkpoint，保留有效设置/成长 | [兼容回归](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/save-compatibility.test.ts)：在输入边界阻止错误比等恢复崩溃更可靠 |
| 首次 Linux CI 5 个开发浏览器用例失败 | 原始结果包含 transition/Boss intro 等待超时、elite 尚未生成、回放 tick=80 未达断言；已修复额外 delta smoothing、合成 focus 与等待条件，本地全量通过；远端结果见验收记录 | [失败 run](https://github.com/QiQiyzhu/arc-shift/actions/runs/34447501573)、[原始错误](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/qa/engineering/ci-first-failure/browser.json)：修复假设需后续全量实跑，不能以本地通过覆盖远端失败 |
...
| # | 核心文件 | 逐行读懂的重点 | 验证抓手 |
|---:|---|---|---|
| P1 | [game/engine.ts](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/game/engine.ts) | `decision` 观察点、`start/resume/enter/update/advance`、暂停原阶段、奖励/消费合法性、`finish` 一次结算 | [经济测试](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/economy.test.ts)、[回归](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/regressions.test.ts) |
| P2 | [game/world.ts](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/game/world.ts) | 所有运行态归属、实体 ID、池槽、RNG、查询指标、spawn 如何消费内容参数 | [战斗测试](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/combat.test.ts)、[内容测试](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/content.test.ts) |
| P3 | [combat/projectiles.ts](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/combat/projectiles.ts) | 池复用字段重置、运动路径、swept 候选、命中顺序、击退更新、generation 与 hits Set | [空间对照](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/spatial-grid.test.ts)、[武器测试](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/weapons.test.ts) |
| P4 | [combat/damage.ts](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/combat/damage.ts) | 护盾/HP/暴击、反应预算、`proc=false`、death guard、掉落与累计伤害 | [共振测试](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/resonance.test.ts)、[回归](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/regressions.test.ts) |
| P5 | [core/spatial-grid.ts](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/core/spatial-grid.ts) | 负坐标 floor、多格覆盖、移动成员维护、去重和原数组顺序；连读 separation 的动态重查询 | [随机布局/高速弹体对照](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/spatial-grid.test.ts) |
| P6 | [replay/replay.ts](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/replay/replay.ts) | 输入与命令全序、初始条件、导入约束、canonical、FNV、停播与 lastGoodTick | [Replay 单元](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/replay.test.ts)、[三份 fixture](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/fixtures/replays) |
| P7 | [content/schema.ts](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/content/schema.ts) | 手写封闭校验、范围与引用、环、规范排序与冻结、语义 diff | [500 组默认等价及消费者测试](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/content.test.ts) |
| P8 | [core/save.ts](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/core/save.ts) | version 1 契约、元进度过滤、checkpoint 合法性、规范路线、存储异常回退 | [存档兼容](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/save-compatibility.test.ts) |
| P9 | [input/actions.ts](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/input/actions.ts) | held 与 edge 分离、无 tick 锁存、重复键抑制、手柄标准映射、断连、准星设备选择 | [输入单元源码](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/input.test.ts)，M7 完整验收已通过 |
| P10 | [cards/system.ts](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/cards/system.ts) | 属性操作顺序、默认浮点等价、奖励独立 RNG、前置约束、共享 grant mutation | [内容](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/content.test.ts)、[共振](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/resonance.test.ts) |

必要的小型依赖同时阅读 [math/RNG](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/core/math.ts)、[Pool](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/core/pool.ts)、[segmentHits](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/combat/rules.ts)、[separation](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/combat/separation.ts)、[enemy-ai](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/ai/enemy-ai.ts)。这不增加 P 的十项计数；它们用于把主路径真正连起来。

## Q. 5 个必须读懂的 Frontend 文件

| # | 表现/工具 UI 文件 | 必须解释的问题 |
|---:|---|---|
| Q1 | [ui/GameApp.tsx](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/ui/GameApp.tsx) | Engine/Synth/Scene 集成、80ms HUD 刷新、设置音频同步、DEV QA、MCP 清理、React StrictMode 生命周期 |
| Q2 | [game/scene.ts](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/game/scene.ts) | 浏览器 delta 与固定步、输入锁存、失焦与面板阻塞、画布绘制读取 World、资源释放；焦点与重连补丁已通过 M7 完整验收 |
| Q3 | [render/actors.ts](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/render/actors.ts) | 角色/Boss/状态可读性与绘制负担；绘制逻辑不直接修改伤害判定 |
| Q4 | [dev/ContentEditor.tsx](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/dev/ContentEditor.tsx) | draft/validated/applied 状态分离、错误与 diff、JSON 往返、隔离沙盒真实数值，不把表单等同于规则编辑器 |
| Q5 | [dev/Debugger.tsx](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/dev/Debugger.tsx) | 只读指标、选中实体/FSM、单步调速、叠加绘制、真实训练 Engine、生产隔离 |

辅助连读 [Replay UI](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/dev/DevApp.tsx)、[DebugSession](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/dev/debug-session.ts)、[ArenaCanvas](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/dev/ArenaCanvas.tsx)、[WebAudio Synth](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/audio/synth.ts) 和 [DEV 入口](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/main.tsx)。没有浏览器表现层的实测，就不能把纯 Node 的正确性扩展成 UI/音频/交互均正确。
...
来源：[src/game/scene.ts](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/game/scene.ts#L198)。
...
验证：[输入用例](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/input.test.ts)、[浏览器用例](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/e2e/input.spec.ts)；M7 最终通过 146 单元 / 34 浏览器，另行 1 生产检查。

### R2. Engine 的输入观察点与逻辑 tick

来源：[src/game/engine.ts](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/game/engine.ts#L297)。
...
验证：[Replay](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/replay.test.ts)、[完整 Engine brute/grid 对照](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/spatial-grid.test.ts)。

### R3. Grid 候选去重并保持原数组顺序

来源：[src/core/spatial-grid.ts](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/core/spatial-grid.ts#L92)。
...
验证：[负坐标、大圆、ID 顺序与高速弹体反例](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/spatial-grid.test.ts)。

### R4. 动态分离中的重新查询边界

来源：[src/combat/separation.ts](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/combat/separation.ts#L35)。
...
验证：[80 个密集随机布局、每组 5 步的精确 oracle](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/spatial-grid.test.ts)。

### R5. 线段—圆的高速弹体 narrow phase

来源：[src/combat/rules.ts](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/combat/rules.ts#L54)。
...
验证：[高速 16000 速度、穿透、击退、分裂的旧算法对照](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/spatial-grid.test.ts)。

### R6. 有界对象池与可观察的耗尽

来源：[src/core/pool.ts](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/core/pool.ts#L11)。
...
验证：[池复用对照](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/spatial-grid.test.ts)、[长模拟原始 miss 计数](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/qa/engineering/soak-long.json)。

### R7. Canonical 状态与诊断 checksum

来源：[src/replay/replay.ts](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/replay/replay.ts#L66)。
...
验证：[Replay 状态范围与篡改反例](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/replay.test.ts)、[Replay 边界说明](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/replay.md)。

### R8. 周期 checksum 失配停播

来源：[src/replay/replay.ts](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/replay/replay.ts#L450)。
...
验证：[Replay 错序、命令结果、周期边界测试](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/replay.test.ts)、[导出/单步浏览器流程](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/e2e/replay.spec.ts)。

### R9. 校验后规范顺序与隔离所有权

来源：[src/content/schema.ts](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/content/schema.ts#L264)。
...
验证：[500 组历史默认输出哈希及参数消费者](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/content.test.ts)；这不等于平衡测试。

### R10. 键盘 held 与短按 edge 分离

来源：[src/input/actions.ts](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/input/actions.ts#L54)。
...
验证：[输入单元源码](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/tests/input.test.ts)；M7 最终 146/34 通过，另行 1 项生产检查，包含断连/持续失焦和无轮询间隙的回归。
...
奖励若共享战斗 RNG，多打一发触发暴击可能改变下一张卡，开关效果也可能影响玩法。当前战斗 RNG 属于 World，房间进入重新设定；奖励和路线/房间生成用由 seed/room 派生的独立 Random，场景布景独立 seed，音频噪声使用独立 `Math.random()`。这是按用途隔离随机消费，不是每个子系统都有完整流注册器；`hitEnemy` 的随机调用次数和顺序仍须保留。[Random 调用点](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/cards/system.ts)、[房间生成](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/rooms/generator.ts)、[路线](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/src/rooms/expedition.ts)。
...
AI 参与主要实现、重构、测试、性能脚本、文档和审查；记录显示不同阶段审查暴露了输入、回放、音频、存档和界面边界问题。不能把大量 AI 生成代码描述为自己无辅助独立手写。本人负责的经历必须按实际参与说明；采用简历中的措辞前，必须能复现测量、改写关键片段并解释失败与取舍。[开发记录](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/ai-development-log.md)。
...
5. **AI-assisted 建立游戏稳定性与故障证据链**，实现 DEV 单步/FSM/池/网格调试、存档兼容回归和六种子共 648,000 tick 的 headless 模拟；该训练夹具零池 miss，保留失败与内存原始记录，明确未证明浏览器墙钟三小时无泄漏。[调试器](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/debugger.md)、[soak 原始结果](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/qa/engineering/soak-long.json)、[边界说明](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/stability.md)。

### 如何采用这些材料

先实际运行项目和命令，再按 P/Q 阅读，手写或口述 R 的核心逻辑并回答 S。每条简历候选都要能回到对应源码和原始报告；根据本人实际承担范围调整动词。真实用户反馈、实体硬件、真实模型和商业效果没有结果时保持未测，不补造经历。公开交付与最新验证链接统一见 [作品介绍](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/portfolio.md) 和 [验收记录](https://github.com/QiQiyzhu/arc-shift/blob/codex/engineering-v2/docs/qa-report.md)。
