...
以下是 [db.py](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/backend/db.py) 实际创建的六张表，不是理想化 ER 图。
...
实际代码见 [engine.py](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/backend/engine.py)。它是可复现的**词汇检索 + 摘录基线**，没有 embedding、向量数据库、重排模型或语义蕴含判断。中文单字匹配和英文词面匹配会遗漏同义表达；原文里出现一句话也不能证明其事实正确。DeepSeek Provider 有真实 HTTP 适配，默认 deepseek-flash；本表的 48 次结果仍来自 extractive。独立 [真实调用配置与证据约定](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/docs/real-model-setup.md) 将模型探针与合成研究结果分开。
...
| 层级 | 实际结果 | 证据 |
|---|---|---|
| 后端 | 63 passed；44 原有检查 + 19 远程适配/探针检查，零付费模型请求 | [本轮 JUnit](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/reports/remote-provider-tests.xml) |
| 浏览器 | 6 passed，真实 FastAPI/SQLite | [本轮报告](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/reports/remote-provider-browser-tests.json) |
| 真实 DeepSeek 小探针 | 3 次响应，2/3 任务契约通过，874 token；非 benchmark | [逐例回执](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/reports/deepseek-smoke.json) |
| 开发夹具 | 12 × 4 = 48 次实际执行 | [原始 JSON](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/reports/evaluation.json) |
| 前端 | TypeScript、lint、production build 通过 | [验证记录](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/docs/validation-report.md) |
| 依赖 | npm audit 0 项已知漏洞，当次快照 | [审计](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/reports/npm-audit.json) |
...
数值来自 [evaluation.md](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/reports/evaluation.md)，数据 SHA256 为 `71ecece501189fd39ec5cb244485247231a5bb06115d31171452bf0bc72131dc`。Workflow 有 10 个待人工审批结果，这不等于已批准。全部是同一套合成开发数据上的确定性执行，human ratings 为 null。**没有独立检索 Recall@K/MRR/nDCG 实验，也没有真实 LLM 的 RAG benchmark。** 因此不能把表里的 0→100% 写成提示工程带来的模型提升。另有 [三次真实 DeepSeek 直接来源调用](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/docs/real-model-results.md)，两例通过、一例过度弃答失败；它没有使用相同四变体处理链，不属于此表的 RAG benchmark。
...
[原始逐请求结果](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/reports/performance-readonly.json) · [复现脚本](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/scripts/benchmark_readonly.py)。HTTP 时间包含数据库读取、序列化和本机调度；这组采样没有单独 DB latency、LLM latency、长时间稳态或跨机器结果。P50 为中位数，P95/P99 为排序后的下界样本位点。这是 255 个短时只读请求的观测，不代表生产容量或复杂工作流吞吐。另有三次真实 DeepSeek 观测：684.770 / 343.148 / 853.852 ms，合计输入 795、输出 79 token；样本太少，未声称生产延迟分位数，账单未知仍为 null。
...
7. 真实 DeepSeek 对 instruction-data 弃答，违反既有“保留敌意材料为引文”的覆盖契约；格式与禁止内容检查通过，整体任务失败。没有泄密观察，也不能由一次弃答证明注入防护完备。[原始结果](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/docs/real-model-results.md)

这些修复的验证范围见 [validation-report.md](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/docs/validation-report.md)。真实用户对流程是否理解、是否愿意使用仍未知。
...
| 文件 | 阅读时必须解释 |
|---|---|
| [backend/app.py](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/backend/app.py) | API 中业务门控为什么不能放在前端 |
| [backend/models.py](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/backend/models.py) | 输入枚举、长度和 effort>0 如何防止非法状态 |
| [backend/db.py](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/backend/db.py) | 参数绑定、WAL、提交/回滚及双写缺口 |
| [backend/importer.py](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/backend/importer.py) | 同意、编码字节限制、解析与来源哈希 |
| [backend/engine.py](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/backend/engine.py) | 词汇排序、严格摘录、快照、审批恢复 |
| [backend/feasibility.py](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/backend/feasibility.py) | 规则建议的边界、No AI 和隐私/延迟约束 |
| [backend/evaluation.py](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/backend/evaluation.py) | 什么算通过，哪些结果是 pending/skipped |
| [backend/analytics.py](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/backend/analytics.py) | 群组、顺序漏斗、空分母 |
| [backend/seed.py](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/backend/seed.py) | 为什么演示标签必须贯穿来源和派生结果 |
| [backend/remote_provider.py](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/backend/remote_provider.py) | DeepSeek 思考开关、请求边界、失败保留与真实用量 |

## Q. 5 个必须读懂的 Frontend 文件

| 文件 | 阅读重点 |
|---|---|
| [App.tsx](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/frontend/src/App.tsx) | 工作区状态、来源导入、错误处理与焦点管理 |
| [WorkspacePanels.tsx](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/frontend/src/WorkspacePanels.tsx) | 机会、可行性、版本、审批、评估与分析的交互 |
| [api.ts](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/frontend/src/api.ts) | 真实接口请求、响应与错误传播 |
| [main.tsx](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/frontend/src/main.tsx) | React 挂载和 StrictMode |
| [style.css](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/frontend/src/style.css) | 信息层级、窄屏、弹窗与滚动边界 |
