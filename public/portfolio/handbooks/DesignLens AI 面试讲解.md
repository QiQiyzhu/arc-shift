...
实际代码见 [engine.py](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/backend/engine.py)。它是可复现的**词汇检索 + 摘录基线**，没有 embedding、向量数据库、重排模型或语义蕴含判断。中文单字匹配和英文词面匹配会遗漏同义表达；原文里出现一句话也不能证明其事实正确。可选 Provider 有真实 HTTP 适配，但本报告没有调用它。
...
| 层级 | 实际结果 | 证据 |
|---|---|---|
| 后端 | 43 passed | [JUnit](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/reports/backend-tests.xml) |
| 浏览器 | 6 passed，真实 FastAPI/SQLite | [报告](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/reports/browser-tests.json) |
...
[原始逐请求结果](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/reports/performance-readonly.json) · [复现脚本](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/scripts/benchmark_readonly.py)。HTTP 时间包含数据库读取、序列化和本机调度；没有单独 DB latency、LLM latency、长时间稳态或跨机器结果。P50 为中位数，P95/P99 为排序后的下界样本位点。这是 255 个短时只读请求的观测，不代表生产容量或复杂工作流吞吐。
...
| [analytics/analyze.py](https://github.com/QiQiyzhu/designlens-ai/blob/codex/designlens-v1/analytics/analyze.py) | CLI 参数如何选择数据库与群组并落盘 |
