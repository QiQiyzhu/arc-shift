...
Actual JOIN/GROUP BY and metrics SQL is in [operations.sql](https://github.com/QiQiyzhu/opspilot-ai/blob/codex/opspilot-v1/analytics/sql/operations.sql). Four real `EXPLAIN (ANALYZE, BUFFERS, FORMAT JSON)` plans are saved in [sql-explain.json](https://github.com/QiQiyzhu/opspilot-ai/blob/codex/opspilot-v1/evals/reports/sql-explain.json); run `python -m analytics.explain` after loading your database to regenerate them. They contain measured timings and planner choices, not hypothetical output.
...
见 [RAG设计](https://github.com/QiQiyzhu/opspilot-ai/blob/codex/opspilot-v1/docs/rag.md)。Parse→Clean→Section→Chunk→Metadata→真实ONNX→PGvector/BM25→RRF→词汇重排→有效版本证据。

## E. Agent workflow

见 [Agent](https://github.com/QiQiyzhu/opspilot-ai/blob/codex/opspilot-v1/docs/agent.md)。结构化意图+有界工具→proposal→服务端人审→transaction→独立Verifier→答复；private CoT不记录。
...
Local backend: **69 passed**, failures=0, errors=0, skipped=0, runtime 9.088s. [JUnit](https://github.com/QiQiyzhu/opspilot-ai/blob/codex/opspilot-v1/evals/reports/backend-junit.xml). One upstream Starlette/AnyIO deprecation warning was observed. Tests include actual PostgreSQL, ONNX, TCP MCP, SSE and transaction concurrency.

Frontend actual result: **7 unit tests passed; 8 browser scenarios passed, 0 skipped, 0 flaky**, runtime 53.86s. [Runner evidence](https://github.com/QiQiyzhu/opspilot-ai/blob/codex/opspilot-v1/docs/assets/frontend-validation.json) also records build/typecheck/lint success and npm audit 0. The continuous actual browser recording is 33.36s with 0 page errors, bound to run `c968d69b029641c9b9d635f0e411e564`. This is local Edge evidence, not a remote CI claim.

After the cancellation/ledger verifier fixes, the existing cancellation and refund/idempotency browser scenarios were rerun against the actual backend: **2 passed, 0 failed, 0 skipped**, in 41.41s. [Separate regression report](https://github.com/QiQiyzhu/opspilot-ai/blob/codex/opspilot-v1/evals/reports/browser-approval-regression.json). The earlier eight-scenario report, screenshots and continuous recording remain preserved; this narrower rerun is not presented as a new eight-scenario run. [Actual gate command results](https://github.com/QiQiyzhu/opspilot-ai/blob/codex/opspilot-v1/evals/reports/gate-validation.json) include an accepted full harness and an intentionally rejected 27/60 baseline.
...
| --- | --- | --- |
| Backend pytest against PostgreSQL and live API/MCP/SSE | 69 passed; 0 failed/errors/skipped; 4.761 seconds | [Downloaded JUnit](https://github.com/QiQiyzhu/opspilot-ai/blob/codex/opspilot-v1/evals/reports/linux-backend-junit.xml) |
...
| Chromium browser integration | 8 passed; 0 skipped/failed/flaky; 21.29 seconds | [Downloaded Playwright report](https://github.com/QiQiyzhu/opspilot-ai/blob/codex/opspilot-v1/evals/reports/linux-browser-results.json) |
| CI agent smoke — **FakeModelProvider rules harness** | 10/10 synthetic tasks; offline gate passed; not an LLM score | [CI smoke output](https://github.com/QiQiyzhu/opspilot-ai/blob/codex/opspilot-v1/evals/reports/linux-agent-smoke.json) |
...
`python -m analytics.load_test` opens 10, 25 and 50 concurrent support sessions against the actual API and PostgreSQL. Each session reads an order, performs a bound SQL count, retrieves evidence through the API, creates a conversation/run and polls until terminal completion. It uses **FakeModelProvider**, not an online model. Raw per-session timings, errors, P50/P95/P99 and wall duration are in [performance.json](https://github.com/QiQiyzhu/opspilot-ai/blob/codex/opspilot-v1/evals/reports/performance.json).
