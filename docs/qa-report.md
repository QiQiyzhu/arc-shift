...
日期：2026-09-08。此记录区分规则测试、真实浏览器输入、开发夹具与自动控制器，避免把不同证据混为“真人完整通关”。

## 已通过的验证

| 验证 | 结果与覆盖 |
| --- | --- |
| Vitest | 37 / 37；伤害、扫掠、反弹、无敌、技能、状态、Boss 阶段、池、RNG、卡牌、存档、幂等结算、介绍暂停 |
| Playwright | 6 / 6；真实键鼠、声音解锁、菜单与档案、奖励/路线、精英/治疗、续玩、两 Boss、胜负重开、移动布局、压力样本 |
| 规则完整流程 | 18 / 18 胜利；3 种子 × 3 初始协议 × 2 路线，合法选卡与输入，没有直接清场 |
...
| 依赖 | 交付前 `npm audit --omit=dev` 报告 0 个已知漏洞；这是扫描时快照 |
...
| 2：界面与构筑 | 1440×900 检查主界面、三选一卡面、五系各六张档案、路线与设置；检查刷新续玩、奖励冻结与护盾保留 | [菜单](screenshots/menu.png)、[卡牌](screenshots/draft.png)、[路线](screenshots/map.png) |
...
- [x] 30 项协议、五系构筑、同系共鸣、三选一奖励。
- [x] 随机路径、四个装饰模板、治疗与宝藏房。
- [x] 胜利、死亡、再次开始、检查点续玩、设置保存。
- [x] 合成音频、对象池、规则与浏览器测试、生产构建。
- [x] README、设计、架构、性能、面试、AI 日志、许可证。

公网发布使用 README 中的固定试玩地址；部署完成后的独立 HTTP 与浏览器启动检查单独执行，不计入上述本地 6 组测试。

可运行 `node scripts/smoke.mjs URL` 对本地生产预览或已部署地址复现启动、输入、暂停、存档与资源加载检查。
...
静态房间边界相同，没有障碍寻路或迷宫生成。移动端没有战斗触控；未验证手柄和全部浏览器。未进行长时内存浸泡测试或玩家群体难度测试。压力样本约 38.6 FPS，60 FPS 目标未达成；详见 [性能记录](performance.md)。
